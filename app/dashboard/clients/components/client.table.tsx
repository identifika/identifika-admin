...
async function fetchClients(
    ...
    const res = await fetch(`http://localhost:3000/api/clients?search=${search}&page=${page}&limit=${limit}`);
...
export default  function ClientTable(props: ClientTableProps) {
    ...
    useEffect(() => {
        fetchClients(props.page, props.limit, props.search).then((data) => {
            setData(data);
        });
                                                ...
                                                <td className="px-4 py-4 text-sm font-medium whitespace-nowrap">
                                                    {
                                                        (() => {
                                                            if (client.external_token == null || client.external_token == "") {
                                                                ...
                                                                return (
                                                                    <Link href={`/dashboard/clients/detail/`}>
                                                                        <h2 className="font-medium text-gray-800 dark:text-white ">
                                                                            {client.client_name}
                                                                        </h2>
                                                                        <p className="text-sm font-normal text-gray-600 dark:text-gray-400">
                                                ...
                                                <td className="px-4 py-4 text-sm whitespace-nowrap">
                                                    {
                                                        (() => {
                                                            if (client.external_token == null || client.external_token == "") {
                                                                return (
                                                                    <GenerateTokenDialog clientId={client.id} />
                                                                )
                                                            } else {
                                                                return (
                                                                    <span className="text-gray-800 dark:text-white">
